...
export class SlashBuilder {
    ...
    option(
        option: SlashOptionCallable | ApplicationCommandOption
    ): SlashBuilder {
...
export function transformApplicationCommand(
    ...
    if (typeof cmd.type === "string") {
        cmd.type =
            ApplicationCommandType[
                cmd.type as keyof typeof ApplicationCommandType
            ]
    }
...
export class ApplicationCommandPermissionsManager {
    ...
    async all(
        ...
        const data = await this.rest.api.applications[
            this.slash.getID()
        ].guilds[
    ...
    async get(
        ...
        const data = await this.rest.api.applications[
            this.slash.getID()
        ].guilds[typeof guild === "string" ? guild : guild.id].commands[
            typeof cmd === "object" ? cmd.id : cmd
        ].permissions.get()
