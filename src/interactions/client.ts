...
export class InteractionsClient extends HarmonyEventEmitter<InteractionsClientEvents> {
    ...
    constructor(options: SlashOptions) {
        ...
        if (id === undefined) {
            throw new Error(
                "ID could not be found. Pass at least client or token"
            )
        }
        this.id = id
    ...
    private _getAutocompleteHandler(
        ...
        return [
            ...
            const matched =
                groupMatched && subMatched && nameMatched && optionMatched
            return matched
        })
    }
    ...
    async verifyServerRequest(req: {
        ...
        try {
            ...
            res._httpRespond = async (
                d: InteractionResponsePayload | FormData
            ) =>
                await req.respond({
                    status: 200,
                    headers: new Headers({
