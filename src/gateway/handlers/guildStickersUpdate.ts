...
export const guildStickersUpdate: GatewayEventHandler = async (
    ...
    if (guild !== undefined) {
        ...
        const updated: Array<{
            before: MessageSticker
            after: MessageSticker
        }> = []
        ...
        for (const raw of d.stickers) {
            ...
            if (has === undefined) {
                ...
                const sticker = (await guild.stickers.get(
                    stickerID
                )) as MessageSticker
                added.push(sticker)
        ...
        for (const sticker of stickers.values()) {
            ...
            if (find === undefined) {
                ...
                const foundID = (
                    find.id !== null ? find.id : find.name
                ) as string
                const before = (await guild.stickers.get(
                    foundID
                )) as MessageSticker
                await guild.stickers.set(foundID, find)
                const after = (await guild.stickers.get(
                    foundID
                )) as MessageSticker
