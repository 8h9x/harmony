    ...
    private async onmessage(event: MessageEvent): Promise<void> {
        ...
        switch (op) {
            ...
            case GatewayOpcodes.HEARTBEAT_ACK:
                ...
                this.debug(
                    `Received Heartbeat Ack. Ping Recognized: ${this.ping}ms`
                )
                break
            ...
            case GatewayOpcodes.INVALID_SESSION:
                ...
                this.debug(
                    `Invalid Session received! Resumeable? ${
                        d === true ? "Yes" : "No"
                    }`
                )
                if (d !== true) {
    ...
    private async onclose({ reason, code }: CloseEvent): Promise<void> {
        ...
        switch (code) {
            ...
            case GatewayCloseCodes.DECODE_ERROR:
                throw new Error(
                    "Invalid Payload was sent. This shouldn't happen!"
                )
            case GatewayCloseCodes.NOT_AUTHENTICATED:
                throw new Error(
                    "Not Authorized: Payload was sent before Identifying."
                )
            ...
            case GatewayCloseCodes.INVALID_API_VERSION:
                throw new Error(
                    "Invalid API Version was used. This shouldn't happen!"
                )
