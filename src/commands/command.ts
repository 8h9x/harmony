...
export class CommandsLoader {
    ...
    async load(
        ...
        if (Cmd === undefined)
            throw new Error(
                `Command not exported as ${exportName} from ${filePath}`
            )
...
export class CommandsManager {
    ...
    fetch(parsed: ParsedCommand, bypassDisable?: boolean): Command | undefined {
        ...
        if (cmd === undefined && parsed.args.length > 0) {
            ...
            if (cmd === undefined || cmd.extension?.subPrefix === undefined)
                return
            if (
                this.client.caseSensitive === true
        ...
        if (parsed.args.length !== 0 && cmd.subCommands !== undefined) {
            const resolveSubCommand = (command: Command = cmd!): Command => {
                ...
                if (this.client.caseSensitive !== true)
                ...
                if (sub !== undefined) {
                    const shifted = parsed.args.shift()
                    if (shifted !== undefined)
                        parsed.argString = parsed.argString
                            .slice(shifted.length)
                            .trim()
        ...
        if (typeof search === "string")
            ...
            exists =
                this.find(
                    search.name,
                    subPrefix === undefined
                        ? search.extension?.subPrefix
                        : subPrefix
                ) !== undefined

            if (search.aliases !== undefined) {
                const aliases: string[] =
                    typeof search.aliases === "string"
                        ? [search.aliases]
                        : search.aliases
                exists =
                    aliases
