...
export class CommandClient extends Client implements CommandClientOptions {
    ...
    constructor(options: CommandClientOptions) {
        ...
        this.allowBots =
            options.allowBots === undefined ? false : options.allowBots
    ...
    async processMessage(msg: Message): Promise<any> {
        ...
        if (mentionPrefix) {
            if (msg.content.startsWith(this.user?.mention as string) === true)
                ...
                msg.content.startsWith(this.user?.nickMention as string) ===
                true
            )
        ...
        if (command === undefined)
            return this.emit("commandNotFound", msg, parsed)
        const category =
        ...
        const isChannelBlacklisted = await this.isChannelBlacklisted(
            msg.channel.id
        )
        if (isChannelBlacklisted) return this.emit("commandBlockedChannel", ctx)

        if (msg.guild !== undefined) {
            const isGuildBlacklisted = await this.isGuildBlacklisted(
                msg.guild.id
            )
        ...
        const anyoneCanUseAnyCommandAt =
            botCooldowns.global + this.globalCooldown
        ...
        if (
            ...
            return this.emit(
                ...
                ctx,
                (forCommand
                    ? anyoneCanUseCommandAt
                    : anyoneCanUseAnyCommandAt) - Date.now(),
                forCommand
