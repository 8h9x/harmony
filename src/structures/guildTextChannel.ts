...
export class GuildTextBasedChannel extends GuildTextBasedChannelSuper {
    ...
    async bulkDelete(
        ...
        if (Array.isArray(messages))
            ...
            if (list.length < messages)
                list = (await this.fetchMessages()).array()
            ids = list
        ...
        if (ids.length < 2 || ids.length > 100)
            throw new Error(
                "bulkDelete can only delete messages in range 2-100"
            )

        await this.client.rest.api.channels[this.id].messages[
            "bulk-delete"
        ].post({
            messages: ids
        })
