...
import { MessageStickerFormatTypes } from "../types/channel.ts"
...
export class MessageStickerItem extends SnowflakeBase {
    ...
    private getStickerFileExtension(
        formatType: MessageStickerFormatTypes
    ): string {
    ...
    getUrl(id: string, formatType: MessageStickerFormatTypes): string {
        return `${
            formatType === MessageStickerFormatTypes.GIF
                ? Constants.DISCORD_MEDIA_URL
                : Constants.DISCORD_CDN_URL
        }/stickers/${id}.${this.getStickerFileExtension(formatType)}`
    }
}
...
export class MessageSticker extends SnowflakeBase {
    ...
    async edit(options: Partial<ModifyGuildStickerOptions>): Promise<this> {
        ...
        const { id } = await this.client.stickers.edit(
            this.guildID,
            this,
            options
        )
...
export class MessageStickerPack extends SnowflakeBase {
    ...
    readFromData(data: MessageStickerPackPayload): void {
        this.stickers = data.stickers.map(
            (e) => new MessageSticker(this.client, e)
        )
        this.name = data.name ?? this.name
