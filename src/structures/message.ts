...
export class Message extends SnowflakeBase {
    ...
    async updateRefs(): Promise<void> {
        ...
        const newVal = await this.client.channels.get<TextChannel>(
            this.channelID
        )
        ...
        if (
            (this.channel as unknown as GuildTextBasedChannel).guild !==
            undefined
        ) {
            this.guild = (
                this.channel as unknown as GuildTextBasedChannel
            ).guild
        }
    ...
    async startThread(options: CreateThreadOptions): Promise<ThreadChannel> {
        if (this.channel.isGuildText() === true) {
            ...
            throw new Error("Threads can only be made in Guild Text Channels")
    }
...
export class MessageAttachment {
    ...
    static async load(
        ...
        if (filename === undefined) {
            ...
            if (split !== undefined)
                filename = split.split("?")[0].split("#")[0]
