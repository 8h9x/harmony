...
export class GuildChannel extends Channel {
    ...
    async permissionsFor(target: Member | Role | string): Promise<Permissions> {
        ...
        return target.permissions
            ...
            .remove(
                roleOWs.length === 0 ? 0 : roleOWs.map((e) => Number(e.deny))
            )
            .add(roleOWs.length === 0 ? 0 : roleOWs.map((e) => Number(e.allow)))
            .remove(
                memberOWs.length === 0
                    ? 0
                    : memberOWs.map((e) => Number(e.deny))
            )
            .add(
                memberOWs.length === 0
                    ? 0
                    : memberOWs.map((e) => Number(e.allow))
            )
    }
    ...
    async removeOverwrite(
        ...
        if (
            this.permissionOverwrites.find((e) => e.id === target) === undefined
        )
            throw new Error("Permission Overwrite not found")
        const overwrites = this.permissionOverwrites.filter(
            (e) => e.id !== target
        )
        return await this.edit({ permissionOverwrites: overwrites })
    }
        ...
        if (
            ...
            switch (overwriteAllow) {
                case OverrideType.ADD: {
                    const originalAllow = new Permissions(
                        overwrites[index].allow
                    )
