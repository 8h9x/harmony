...
export class GuildBans extends Base {
    ...
    async get(user: string | User): Promise<GuildBan> {
        ...
        if (typeof res !== "object")
            throw new Error("Failed to fetch Guild Ban")
    ...
    async add(
        ...
        if (res.response.status !== 204)
            throw new Error("Failed to Add Guild Ban")
    }
...
export class Guild extends SnowflakeBase {
    ...
    constructor(client: Client, data: GuildPayload) {
        ...
        this.emojis = new GuildEmojisManager(
            this.client,
            this.client.emojis,
            this
        )
        ...
        this.commands =
            typeof this.client.interactions === "object"
                ...
                : (this.client as unknown as InteractionsClient).commands.for(
                      this
                  )
    }
    ...
    readFromData(data: GuildPayload): void {
        ...
        if (!this.unavailable) {
            ...
            this.widgetChannelID =
                data.widget_channel_id ?? this.widgetChannelID
    ...
    async chunk(
        ...
        return await new Promise((resolve, reject) => {
            this.client.shards
                .get(this.shardID)
                ?.requestMembers(this.id, options)
    ...
    getWidgetImageURL(
        ...
        return `https://discord.com/api/v${
            this.client.rest.version ?? 8
        }/guilds/${this.id}/widget.png${
    ...
    async createTemplate(
        ...
        const payload = await this.client.rest.api.guilds[
            this.id
        ].templates.post({
            name,
            description
    ...
    async fetchAuditLog(
        ...
        const data = await this.client.rest.endpoints.getGuildAuditLog(
            this.id,
            {
                userId:
