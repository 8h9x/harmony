...
export class ShardManager extends HarmonyEventEmitter<ShardManagerEvents> {
    ...
    get ping(): number {
        return (
            ...
            this.list.map((e) => e.ping).reduce((p, a) => p + a, 0) /
            this.list.size
        )
    }
    ...
    async getShardCount(): Promise<number> {
        ...
        if (this.cachedShardCount !== undefined)
            shardCount = this.cachedShardCount
        else {
            if (
                ...
                this.debug(
                    `Fetch /api/v${this.client.rest.version}/gateway/bot...`
                )
                ...
                this.debug(
                    `Reset After: ${info.session_start_limit.reset_after}ms`
                )
    ...
    async connect(): Promise<ShardManager> {
        ...
        this.debug(
            `Launching ${shardCount} shard${shardCount === 1 ? "" : "s"}...`
        )
        ...
        this.debug(
            `Launched ${shardCount} shards! Time taken: ${Math.floor(
                diff / 1000
            )}s`
        )
        return this
